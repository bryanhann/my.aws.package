...
from awscli.aws.ec2.create_key_pair import CREATE_KEY_PAIR
from awscli.invoking import Profile
from awscli.invoking import Meta
...
def test_profile():
    ...
    The Meta object:
        ...
        (2) provides the resultingt stdout and stderr as strings
...
def test_meta():
    ...
    # stdout and stderr should be 'bytes'
    assert type(it.stdout) == type(b"bytes")
    assert type(it.stderr) == type(b"bytes")

    # meta provides the stdout and stderr as strings.
    assert meta.stdout == it.stdout.decode('utf-8')
    assert meta.stderr == it.stderr.decode('utf-8')
    ...
    assert meta._wrapper         == CREATE_KEY_PAIR
    assert meta._wrap.__class__  == CREATE_KEY_PAIR
